...
### Using Xacro to clean up your code (1h)
- `Xacro` package may helps to reduce amount of code in a URDF file
- 3 things that may helps:
    - `Constants`
    - `Simple Maths`
    - `Macros`
...
#### Using Xacro
- `Xacro`
    - It is a macro language for XML
    - Runs all macros and outputs the results
...
- To `xacro` tool parse properly an `URDF` file, the file need to include spicify a *namespace*
```xml
    <?xml version="1.0"?>
...
- `Launch file`
    - Can be used to automatically generate *URDF* files
    - To run `xacro` within need to put `Command` as a *parameter*
    - Example as parameter to `robot_state_publisher`
```py
...
```
   ...
   - `urdf_launch` package let automatically load robot model (xacro/urdf)
```py
...
#### Constants
- `xacro` allows specify properties which act as constants.
- Example
    - Values that are repeated in different parts of the URDF file may be defined as xacro contants:
```xml
...
```
- `${}` construct are then used to replace the `${}`

#### Math
- Lets build up complex expressions using:
    - `${}` 
    - 4 basic operators `+`, `-`, `*`, `/`
    - *Unary minus* and *parenthesis*
    - Other more complex operators like `sin`, `cos`, etc

#### Macros
- Biggest and most useful component
...
- Every instance of `<xacro:$NAME />` is replaced with content of `xacro:macro`
- *Note: If xacro with a specified name is not found, it will not NOT generate an error*

##### Parameterized Macro
- Parameterize macros let that macros not generate exact same text everytime
- Example, basic:
```xml
...
- `block` parameter:
    - To specify a block parameter, include an `*` before its parameter name
    - Can be inserted using `insert_block` command
...
#### Practical Usages
- Launch model generated by xacro, using launch file:
```bash
    # Init environment
    cd ~/learnRobotic/ && source ros2_env_conf.sh && cd ros2_ws && source install/setup.bash
    # Launch model '01-myfirst.urdf'
...
##### Leg macro
- Macro and simple mahs let create multiple similar objects in different locations
- Exampple, legs of R2D2
```xml
...
- Tricks:
    - Use a *prefix* to get 2 similarly named objects
    - Use *maths* to calculate joints origin
    - Use *reflect* parameter and setting to `1` or `-1`

### Using URDF with robot_state_publisher - C++ (2h)
- Describe in URDF file a walking robot model
- Write a node which simulate motion and publises `JoinState` and `transforms`
- Use `robot_state_publisher` to publish robot state to `/tf`

#### 1 Create package `urdf_tutorial_cpp`
```bash
    # Init environment
    cd ~/learnRobotic/ && source ros2_env_conf.sh && cd ros2_ws/src
    # Build package and go to package
...
- Create folder:
```bash
    cd tf2_geometry_msgs
    mkdir -p urdf
```

- Add to `tf2_geometry_msgs/urdf`:
    - Robot model: `r2d2.urdf.xml`
    - Rviz configuration: `r2d2.rviz`

#### 3 Publish state
- Add source for publish robot state `urdf_tutorial_cpp/src/urdf_tutorial.cpp`:
    - Publish `JointState` message to `/joint_state` topic, so that `robot_state_publisher` can:
        - Compute per-joint transforms
        - Broadcast them via `/tf`
    - Broadcast single root transform that places robot model (`axis` frame) in world (`odom` frame), making robot walk in circle.

#### 4 Create launch file
- Add launch file `urdf_tutorial_cpp/launch/launch.py`

#### 5 Edit CMakeLists.txt file
- Add dependencies
- Add executable
- *install(DIRECTORY urdf ...)* rule copies both `r2d2.urdf.xml` and `r2d2.rviz` into the install tree so they can be found at runtime
...
#### 7 View result
- Run executables with launch file
```bash
    # Init environment
    cd ~/learnRobotic/ && source ros2_env_conf.sh && cd ros2_ws && source install/setup.bash
    # Run launch file
...
- Visualize results in rivz2. New terminal
```bash
    # Init environment
    cd ~/learnRobotic/ && source ros2_env_conf.sh && cd ros2_ws && source install/setup.bash
    # Run rviz2
...
### Using URDF with robot_state_publisher - Python (30')
- Create URDF model of movile robot
- Write node which simulates motion and publishes `JointState` `transform`
- Use `robot_state_publisher` to publish robot state `/tf2`
...
```bash
    # Move inside package
    cd cd ~/learnRobotic/ros2_ws/src/urdf_tutorial_r2d2
    # Create folder for urdf files
    mkdir -p urdf
```
- Inside `urdf` folder insert downloaded files:
    - `r2d2.rviz`: URDF file
    - `r2d2.urdf.xml`:  Rviz configuration file

#### 3 Publish state
- To speicify state of robot we need to specify all 3 joints and overall odometry
- Add source of node that publish this `state_publisher.py` inside proper path: `urdf_tutorial_r2d2/urdf_tutorial_r2d2/state_publisher.py`

#### 4 Create launch file
- Create launch file: `second_ros2_ws/src/urdf_tutorial_r2d2/launch/demo_launch.py`

#### 5 Edit `setup.py` file
...
```
    - Append `launch` and `urdf` folders
```py
    data_files=[
        ...
        (os.path.join('share', package_name, 'launch'), glob('launch/*')),
...
```
    - Add to entry_point the `state_publisher`
```py
    'console_scripts': [
